...
const AcceptedRequest = ({confirmedRequests,cancelledRequests,handleCancel}:IAcceptedRequestProps) => {
  const [open, setOpen] = useState(false);
  const [curReq, setCurReq] = useState<Request>({id: 0,customerName: "",tripName: "",startDate: "",endDate: "",price:0,numCustomer:0,status:0 });
  const handleClick = (request:Request) => {
    setCurReq(request);
    setOpen(true);
  };
        ...
        <Grid container direction={{xs:"column",sm:"row"}} justifyContent={{xs:"flex-start",sm:"space-between"}}>
            <Grid item>
              ...
              <Grid item container xs="auto" direction={{xs:"row",sm:"column"}} justifyContent={{xs:"flex-start",sm:"center"}} alignItems={{xs:"center",sm:"flex-end"}}>
                  <Button variant="contained" color="warning" onClick={()=>handleClick(request)} size="small" sx={{marginRight:"2vw", marginTop:"1vh"}} >
                        Cancel
                  </Button>
  ...
  return (
    <div>
      ...
      <Dialog open={open} onClose={handleNo}>
        <DialogTitle>Confirmation</DialogTitle>
        <DialogContent>
          <Typography variant='body2' color="textSecondary">Upon cancellation, the client's money we be fully refunded.</Typography>
          <Typography variant='body2' color="textSecondary">However, you may lost some of your credibility.</Typography><br/>
          <Typography variant='body2' color="textSecondary"> This process cannot be undone, are you sure you want to cancel this appointment?</Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleYes} color="primary" size="small">
            Yes
